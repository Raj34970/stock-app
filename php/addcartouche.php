...
<body>
    ...
    <div class="login-form">
        <form action="" method= "POST">
            ...
            <h1>ADD-Cartouche</h1>
            <div class="show">
                <?php echo htmlentities("Total: ".$count_car);?>
            </div>
            <div class="content">
                ...
                <div class="input-field">
                    ...
                    <select name="color">
                        ...
                        <option name="color">M</option>
                        <option name="color">Y</option>
                        <option name="color">K</option>
                    </select>
                </div>
                <div class="input-field">
...
if(isset($_POST['submit'])){
    ...
    if(empty($name) ||empty($ref)|| empty($color)|| empty($quantity)){
        ?><script>swal.fire("Attention", "All feilds required", "warning")</script><?php
        mysqli_close($conn);
    }
    else{
        $sql =mysqli_query($conn, "SELECT * from cartouche");
        while($row=mysqli_fetch_array($sql)){
            $st_id=$row["id"];
            if($row["ref"]==$ref and $quantity>0){
                #echo htmlentities($row["ref"]);
                $t=$quantity+$row["quantity"];
                $update_tb= "UPDATE cartouche SET quantity='$t' WHERE id=$st_id";
        ...
        if($i==0){
            $query ="insert into cartouche(date,time,name,ref,color,quantity) values('$date','$time','$name','$ref','$color','$quantity')";
            $run = mysqli_query($conn,$query);
            ?><script>swal.fire("Perfect", "added", "success")</script><?php
        }else{
            ?><script>swal.fire("Good Job", "Updated", "success")</script><?php
        }
    }
}
?>
